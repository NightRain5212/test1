...
> 很多领域都和web捆绑在一起，但是快节奏情况下很容易被AI误导
...
**既然对于小项目根本无法像大型项目一样彻底将接口文档写死，那么就采用**<span style="color:red">解耦合的方式处理各个需要存储的文件数据。</span>
具体来说，例如要处理用户头像，那么就**前后端单独写一个函数**负责其增删查改，**在store中单独开辟变量管理其在前端的存在**,在login的时候也**不再考虑提前通过getinfo获取**，而是当需要的时候再调用后端获取数据。这样一来后端就可以很**灵活**地管理前端多变的数据结构了。变更的时候也只需要改那几个函数，而不需要更改前端的其他代码。
<span style="color:red">否则后端非常乱。</span>

星火大模型的1token约1.5字节,并且问题和回答都算在内。。。省着点用。。。
...
这个应用涉及到很多模型，所以每个环节的模型实现的功能都最好测试完成再集成。

## 功能目标

1.主要功能:默认单人模式:先提交简历(支持纸质扫描成电子版),并且简历只提取关键信息，不会考虑美观程度等因素,然后在完整一次面试过程中由ai自动根据简历内容生成问题。结束后生成报告，并且可以保存记录。如果未完成一次完整面试则不支持保存。报告的内容格式待定。
多人模式:待定
(附)未点击开始面试时，也可以映入镜头获取建议.

2.历史记录:按时间排序。对于每条记录，除了显示相关的时间节点，还显示总评(优秀、良好等).然后可以点击查看当时的录制视频、或者听音频、看当时提交的简历、更详细的报告(pdf/doc等)，可以在线预览也可以下载到本地。<span style="color:red">考虑到面试时长高度30-60分钟，因此历史记录只支持展示前x条(例如前十条)</span>
